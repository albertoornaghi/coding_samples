...
if `word_count'==1{
	...
	***generate variable with only languages that have more that 50 observations
	...
	gen language_sample_above=""
	gen language_sample_below=""
	...
	***insert languages with over 50 observations into new variable
	...
	foreach x of local languages{
		...
		replace language_sample_above=language if `r(N)'>200 & language_sieve=="`x'"
	...
	*** graph duration per word over languages
	...
	foreach x of local languages{
		...
		count if language_sieve=="`x'"
		replace language_sample_below=language if `r(N)'<=200 & language_sieve=="`x'"
	...
	***create graph of R7 to R9 differences
	...
	preserve
		...
		***bring R7 and R9 means into the same observations over different variables
		...
		ytitle("Difference between Round 7 and Round 9 duration (mins)") sort(diff) yline(0) ///
		blab(bar, size(vsmall) format(%7.1f) pos(out)) name("`x'", replace) bar(1, color(stgreen)) 
		}
		
		if "`var'"=="duration_per_q_seconds"{
		graph bar diff, over(country_count, sort(1) label(labsize(vsmall))) horiz ylabel(,nogrid) ///
		ytitle("Difference between Round 7 and Round 9 question duration (secs)") sort(diff) ///
		yline(0) blab(bar, size(vsmall) format(%7.1f) pos(out)) name("`x'", replace)
		}
...
if `questions'==1{
	...
	*** create graphs that plot mean interview duration against the number of questions asked
	...
	preserve
		...
		label var total_q "Total question items"
	
		label define countries_scatter 0 "BFO" 1 "BEN" 2 "BOT" 3 "CAM" 4 "CDI" 5 "CVE" 6 "ESW"  7 "GAB" 8 "GAM" ///
		9 "GHA" 10 "GUI" 11 "KEN" 12 "LES" 13 "LIB" 14 "MAD" 15 "MAU" 16 "MLI" 17 "MLW" 18 "MOR" 19 "MOZ" ///
		20 "NAM" 21 "NGR" 22 "NIG" 23 "SAF" 24 "SEN" 25 "SRL" 26 "STP" 27 "SUD" 28 "TAN" 29 "TOG" 30 "TUN" ///
		31 "UGA" 32 "ZAM" 33 "ZIM" 34 "ANG" 35 "ETH" 36 "CBZ" 37 "MTA" 38 "SEY"
	
		label values country_count countries_scatter
	...
	***create bar graph for duration by language with error bars
	...
	foreach x of varlist LENGTH wordcount duration_per_word_seconds{
		...
		preserve
			...
			gen language_order = 1 if language_count==2
			replace language_order = 2 if language_count==3
			replace language_order = 3 if language_count==4
			replace language_order = 4 if language_count==1
			replace language_order = 5 if language_count==5
			
			gen wordcount_order = 1 if language_count==2
			replace wordcount_order = 2 if language_count==1
			replace wordcount_order = 3 if language_count==3
			replace wordcount_order = 4 if language_count==5
			replace wordcount_order = 5 if language_count==4
			
			gen question_order = 1 if language_count== 4
			replace question_order = 2 if language_count == 3
			replace question_order = 3 if language_count == 2
			replace question_order = 4 if language_count == 1
			replace question_order = 5 if language_count == 5
			
			drop if round_9==0

			label define language_order 1 "French" 2 "Portuguese" 3 "Kiswahili" 4 "English" 5 "Arabic"
			label values language_order language_order
			
			label define wordcount_order 1 "French" 2 "English" 3 "Portuguese" 4 "Arabic" 5 "Kiswahili"
			label values wordcount_order wordcount_order
			
			label define question_order 1 "Kiswahili" 2 "Portuguese" 3 "French" 4 "English" 5 "Arabic"
			label values question_order question_order
